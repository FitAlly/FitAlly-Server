...
public class ExerciseController {
    ...
    /** 홈화면 미리보기 */
    @GetMapping("/preview")
    public ResponseEntity<List<ExercisePreviewResponse>> getPreview() {
        return ResponseEntity.ok(exerciseService.getPreview());
    }

    /** 전체보기 (키워드 검색 / 카테고리·난이도 필터) */
    @GetMapping
    public ResponseEntity<List<ExerciseListResponse>> getExercises(
            @RequestParam(required = false) String keyword,
            @RequestParam(required = false) String category,
            @RequestParam(required = false) String difficulty) {
        return ResponseEntity.ok(exerciseService.getExercises(keyword, category, difficulty));
    }

    /** 단건 상세 */
    @GetMapping("/{exerciseId}")
    public ResponseEntity<ExerciseDetailResponse> getExercise(@PathVariable Long exerciseId) {
